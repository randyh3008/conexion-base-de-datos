...
public class Registro extends javax.swing.JFrame {
	...
	public void crearPanel() {
		...
		etiqueta = new JLabel("Crear Usuario");
			...
			if (!area.getText().isEmpty()) {
				if (passText.length() >= 4) {
					if (p1.buscarUsuario("SELECT buscarUsuario('" + area.getText() + "','" + passText + "');")) {
						...
						long id = p1.addUsuario(area.getText(), passText);
						if (id > 0) {
						...
						JOptionPane.showMessageDialog(null, "el nombre de usuario ya existe intente otro", "Alert",
								JOptionPane.WARNING_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(null, "necesita usar una contrase�a mas fuerte", "Alert",
							JOptionPane.WARNING_MESSAGE);
				}
